var app = new Vue({
		...
		axios
			...
			.then((res) => {
				...
				this.weekClass.growingData = resData
					...
					.filter((item) => (!(set.has(item.Title) && set.has(item.classDay)) ? set.add(item.Title) && set.add(item.classDay) : false));
				...
				this.onlineDefault = resData.filter((item) => {
					return item.className === 'sharing' && item.classType === '線上直播' && item.Lesson !== '每月固定課程' && item.classDay !== '每天';
				});
				this.localDefault = resData.filter((item) => {
					return item.className === 'sharing' && item.classType === '實體現場' && item.Lesson !== '每月固定課程' && item.classDay !== '每天';
				});
			})
			.catch((err) => {
	...
	computed: {
		...
		onlineClass() {
			...
			newOnlineData = [...new Set(newOnlineData.map((item) => JSON.stringify(item)))].map((item) => JSON.parse(item));
			newOnlineData
				.sort((a, b) => {
					let aNum = a.classTime ? Number(a.classTime.substr(0, 2)) : 0,
						bNum = b.classTime ? Number(b.classTime.substr(0, 2)) : 0;
		...
		localClass() {
			...
			newLocalData = [...new Set(newLocalData.map((item) => JSON.stringify(item)))].map((item) => JSON.parse(item));
			newLocalData
				.sort((a, b) => {
					let aNum = a.classTime ? Number(a.classTime.substr(0, 2)) : 0,
						bNum = b.classTime ? Number(b.classTime.substr(0, 2)) : 0;
